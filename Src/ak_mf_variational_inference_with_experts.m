function [ ak_network ] = ak_mf_variational_inference_pca_init_with_experts(x,y,L,Q,alpha,M,sigma,gamma,B,landa,ak_const,vi_max_iters)
%
%inputs:
...
nu = theta .* all_etha{1,L};        %fisher vectors of X_L.
...
while(all_converged == false)
    ...
    %-------------------------UPDATE NU and THETA--------------------------
    first_term = all_etha{1,L-1};   %first_term is now NxF
    ...
    %--------------------------------UPDATE P------------------------------
    %general form
    temp = all_etha{1,L};   %temp is now NxK
    temp = -landa*temp;
    temp = 1 + exp(temp);
    temp = 1 ./ temp;
